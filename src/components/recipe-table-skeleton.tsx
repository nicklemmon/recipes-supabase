...
export function RecipeTableSkeleton({ showDietaryPref = true }: RecipeTableSkeletonProps) {
  return (
    <tr className="border-b border-slate-200 dark:border-zinc-800 animate-pulse">
      <td className="p-4">
        <div className="h-5 w-56 bg-gray-200 dark:bg-zinc-800 rounded" />
      </td>
      {showDietaryPref && (
      ...
      <td className="p-4 text-right">
        <span className="md:hidden h-5 w-16 bg-gray-200 dark:bg-zinc-800 rounded inline-block" />
        <div className="hidden md:inline-flex gap-1">
          {Array.from({ length: 5 }).map((_, index) => (
            <div key={index} className="w-4 h-4 bg-gray-200 dark:bg-zinc-800 rounded-full" />
          ))}
        </div>
      </td>
    </tr>
